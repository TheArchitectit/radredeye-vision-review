...
| Godot Addon | `godot/addons/radredeye_capture/` | Screenshot capture |
...
- `radredeye_capture_screenshot` — Trigger immediate capture
...
Copy `godot/addons/radredeye_capture/` into your Godot project's `addons/` directory and enable the plugin in Project Settings. Screenshots are saved to `user://screenshots/` on a configurable interval.
